...
class LuckyALFINet(nn.Module):
    ...
    def __init__(self, arch_config, train_config, simulator, proposal, verbose=3, device="cpu"):
        ...
        if train_config['loss'] not in LuckyALFINet.dict_loss_func.keys():
            raise ValueError("Invalid loss in argument of RIM: {}".format(train_config['loss']))
        if train_config['weight'] not in LuckyALFINet.dict_weight_func.keys():
            raise ValueError("Invalid weight in argument of RIM: {}".format(train_config['weight']))

        self.loss_func = LuckyALFINet.dict_loss_func[train_config['loss']]
        self.weight_func = LuckyALFINet.dict_weight_func[train_config['weight']]
    ...
    def forward(self, X, batch_size_x, batch_size_theta, nb_iter=30, phase="train"):
        ...
        psi_t = self.proposal.init_psi(X.shape[0], self.simulator, self.mu_theta, self.sigma_theta)
    ...
    def test(self, test_config, nb_theta=None, regenerate=False):
        ...
        if regenerate or self.theta_test is None or self.X_test is None:
            ...
            self.mu_Xtest, self.sigma_Xtest, self.X_test = self.normalize(X_test.to(self.device).contiguous().view(-1, X_test.shape[2]))
