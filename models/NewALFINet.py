...
class NewALFINet(nn.Module):
    ...
    def __init__(self, arch_config, train_config, simulator, proposal, verbose=3, device=None):
        ...
        if self.use_grad:
            self.theta_data_agg = DataAggregator(theta_dim*self.proposal.psi_dim
        ...
        if train_config['loss'] not in NewALFINet.dict_loss_func.keys():
            raise ValueError("Invalid loss in argument of RIM: {}".format(train_config['loss']))
        if train_config['weight'] not in NewALFINet.dict_weight_func.keys():
            raise ValueError("Invalid weight in argument of RIM: {}".format(train_config['weight']))

        self.loss_func = NewALFINet.dict_loss_func[train_config['loss']]
        self.weight_func = NewALFINet.dict_weight_func[train_config['weight']]
    ...
    def split_forward(self, X, batch_size_x, batch_size_theta, nb_iter=30, phase="train"):
        ...
        for it in range(nb_iter):
            ...
            if self.use_grad:
                theta_gen = self.proposal.grad_log(theta_gen.permute(1, 0, 2)) \
    ...
    def forward(self, X, batch_size_x, batch_size_theta, nb_iter=30, phase="train"):
        ...
        for it in range(nb_iter):
            ...
            d_psi, st = self.forward_step(x_real, x_gen, theta_gen, st, psi_t.view(x_real.shape[0], -1), phase=phase)
    ...
    def train(self, train_config):
        ...
        for self.epoch in range(self.epoch, nb_epochs):
            ...
            for i_begin_batch in list_batches:
                ...
                if self.split_theta:
                    list_psi_t = self.split_forward(X, train_config['batch_size_x'],
